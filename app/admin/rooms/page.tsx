...
export default function RoomsPage() {
  ...
  const handleDuplicate = useCallback(
    async (id: number) => {
      try {
        setLoading(true);
        await api.post(`/api/hotel/admin/hotel-rooms/${id}/duplicate`);
      ...
      <DataTable
        ...
        columns={[
          ...
          },
          {
            ...
            render: (row: any) => (
              <span
                className={`inline-flex justify-center items-center py-0.5 rounded-full font-medium ${
                  row.status === "Active"
                    ? "bg-green-200 text-green-700"
                    : "bg-red-200 text-red-700"
                }`}
        ...
        onEdit={handleEdit}
        onDelete={handleDeleteRequest}
        onDuplicate={(row: any) => handleDuplicate(row.id)}
      />
